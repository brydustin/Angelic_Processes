...
next
  ...
  next
    ...
    have 2: "x ##\<^sub>S \<Squnion>\<^sub>S (filter (\<lambda>x. x \<notin> set ys) xs)"
      by (smt (verit, best) Cons.prems(1) list.set_intros(1,2) mem_Collect_eq pairwise_compat_foldr pairwise_def
          scene_compats_members set_filter)    
...
next
  ...
  also from assms a have "..."
    ...
    by (smt (verit) Sup_scene_le image_iff subset_iff)
